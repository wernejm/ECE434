...
// Created an additional kthread & modified permissions to allow for two LEDs to blink simultaneously at different rates
...
static unsigned int gpioLED1 = 60;           ///< GPIO for LED1 is 60
module_param(gpioLED1, uint, S_IRUGO);       ///< Param desc. S_IRUGO can be read/not changed
MODULE_PARM_DESC(gpioLED1, " GPIO LED number (60)");     ///< parameter description
...
static unsigned int gpioLED2 = 50;           ///< Default GPIO for the LED is 50
module_param(gpioLED2, uint, S_IRUGO);       ///< Param desc. S_IRUGO can be read/not changed
MODULE_PARM_DESC(gpioLED2, " GPIO LED number (50)");     ///< parameter description
static unsigned int blinkPeriod2 = 1200;
...
static ssize_t period_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf){
   ...
   if (period == blinkPeriod1) {
   	return sprintf(buf, "%d\n", blinkPeriod1); }
   if (period == blinkPeriod2) {
	return sprintf(buf, "%d\n", blinkPeriod2); }
}
...
static struct task_struct *task1;            /// The pointer to the thread task(s)
 ...
 */
static int flash1(void *arg){		// Flash LED1 at a period of blinkPeriod1
...
static int flash2(void *arg){		// Flash LED2 at a period of blinkPeriod2
   printk(KERN_INFO "EBB LED: Thread has started running \n");
   while(!kthread_should_stop()){
      set_current_state(TASK_RUNNING);
      if (mode==FLASH) ledOn2 = !ledOn2;
...
static int __init ebbLED_init(void){
   ...
   sprintf(ledName, "led%d", gpioLED2);      // Create the gpio name for /sys/ebb/led50
   ...
   ledOn1 = true;
   gpio_request(gpioLED1, "sysfs");          // gpioLED is 60 by default, request it
   ...
   ledOn2 = true;
   gpio_request(gpioLED2, "sysfs");          // gpioLED is 50 by default, request it
   ...
   task1 = kthread_run(flash1, NULL, "LED1_flash_thread");  // Start the LED1 flashing thread
   if(IS_ERR(task1)){                                     // Kthread name is LED_flash_thread
      printk(KERN_ALERT "EBB LED: failed to create the task\n");
      return PTR_ERR(task1);
   }
   task2 = kthread_run(flash2, NULL, "LED2_flash_thread");  // Start the LED2 flashing thread
