...
PIN = 1<<16       # P9_30 corresponds to GPIO3 line 16
...
reg_status3 &= ~(PIN)  # set LEDs as outputs
...
try:
  while(True):
    ...
    mem3[GPIO_SETDATAOUT:GPIO_SETDATAOUT+4] = struct.pack("<L", PIN)    # set PIN, sleep a set number of microseconds, then clear PIN and sleep. 
    time.sleep(0.000000001)                                             # 1 ns sleep time
    mem3[GPIO_CLEARDATAOUT:GPIO_CLEARDATAOUT+4] = struct.pack("<L", PIN)
